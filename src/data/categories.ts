import type { Category } from '@/types';

export const categories: Category[] = [
  {
    key: 'stationery',
    slug: 'stationery',
    name: 'Stationery',
    shortName: 'Stationery',
    description:
      'Professional stationery printing for businesses, startups, and individuals in Hyderabad. From business cards to brochures, get everything printed with premium quality and fast turnaround.',
    shortDescription: 'Business cards, flyers, brochures & more',
    image: 'https://placehold.co/800x500/f97316/ffffff',
    icon: 'FileText',
    seoTitle: 'Custom Stationery Printing in Hyderabad | PrintNest',
    seoDescription:
      'Order custom business cards, flyers, brochures, letterheads, and more in Hyderabad. Premium printing with fast delivery.',
  },
  {
    key: 'wedding-parties',
    slug: 'wedding-parties',
    name: 'Wedding & Parties',
    shortName: 'Wedding',
    description:
      'Make your special day unforgettable with custom wedding invitations, acrylic invites, and party stationery. Designed and printed in Hyderabad with love and attention to detail.',
    shortDescription: 'Wedding invites, save-the-dates & celebration cards',
    image: 'https://placehold.co/800x500/be185d/ffffff',
    icon: 'Heart',
    seoTitle: 'Wedding Invitation Printing in Hyderabad | PrintNest',
    seoDescription:
      'Beautiful wedding invitations, acrylic invites, and party cards printed in Hyderabad. Custom designs for your celebration.',
  },
  {
    key: 'clothing-apparels',
    slug: 'clothing-apparels',
    name: 'Clothing & Apparels',
    shortName: 'Clothing',
    description:
      'Custom printed apparel for teams, events, and promotions. T-shirts, polo shirts, hoodies, and more with your branding. Perfect for Hyderabad businesses and organizations.',
    shortDescription: 'T-shirts, polos, hoodies & uniforms',
    image: 'https://placehold.co/800x500/2563eb/ffffff',
    icon: 'Shirt',
    seoTitle: 'Custom T-Shirt & Apparel Printing in Hyderabad | PrintNest',
    seoDescription:
      'Get custom T-shirts, polo shirts, hoodies, and uniforms printed in Hyderabad. Perfect for teams, events, and promotions.',
  },
  {
    key: 'kids-apparels',
    slug: 'kids-apparels',
    name: 'Kids Apparels',
    shortName: 'Kids',
    description:
      'Adorable custom-printed clothing for kids. From baby onesies to polo shirts, create special designs for schools, parties, and family events in Hyderabad.',
    shortDescription: 'Kids polo shirts, t-shirts & baby onesies',
    image: 'https://placehold.co/800x500/8b5cf6/ffffff',
    icon: 'Baby',
    seoTitle: 'Custom Kids Apparel Printing in Hyderabad | PrintNest',
    seoDescription:
      'Custom printed kids clothing in Hyderabad. Polo shirts, T-shirts, and baby onesies with your designs.',
  },
  {
    key: 'bags',
    slug: 'bags',
    name: 'Bags',
    shortName: 'Bags',
    description:
      'Eco-friendly and promotional bags printed with your brand. Tote bags, jute bags, cotton bags, and backpacks perfect for businesses, events, and retail in Hyderabad.',
    shortDescription: 'Tote bags, jute bags, cotton bags & backpacks',
    image: 'https://placehold.co/800x500/059669/ffffff',
    icon: 'ShoppingBag',
    seoTitle: 'Custom Bag Printing in Hyderabad | PrintNest',
    seoDescription:
      'Custom printed tote bags, jute bags, and promotional bags in Hyderabad. Eco-friendly options for businesses and events.',
  },
  {
    key: 'corporate-gifts',
    slug: 'corporate-gifts',
    name: 'Corporate Gifts',
    shortName: 'Corporate',
    description:
      'Impress clients and employees with premium corporate gifts. Custom mugs, water bottles, gift sets, and coasters with your branding. Based in Hyderabad.',
    shortDescription: 'Mugs, bottles, gift sets & coasters',
    image: 'https://placehold.co/800x500/0891b2/ffffff',
    icon: 'Gift',
    seoTitle: 'Corporate Gift Printing in Hyderabad | PrintNest',
    seoDescription:
      'Custom corporate gifts in Hyderabad. Branded mugs, water bottles, gift sets, and coasters for clients and employees.',
  },
  {
    key: 'office-stationery',
    slug: 'office-stationery',
    name: 'Office & Stationery',
    shortName: 'Office',
    description:
      'Essential office supplies with your brand identity. Custom pens, notebooks, diaries, coasters, and employee welcome kits for Hyderabad businesses.',
    shortDescription: 'Pens, notebooks, diaries & welcome kits',
    image: 'https://placehold.co/800x500/64748b/ffffff',
    icon: 'Briefcase',
    seoTitle: 'Custom Office Stationery in Hyderabad | PrintNest',
    seoDescription:
      'Branded office stationery in Hyderabad. Custom pens, notebooks, diaries, and employee welcome kits.',
  },
  {
    key: 'event-solutions',
    slug: 'event-solutions',
    name: 'Event Solutions',
    shortName: 'Events',
    description:
      'Make your event stand out with professional signage and display solutions. Roll-ups, banners, standees, flags, and vinyl stickers for exhibitions, launches, and promotions in Hyderabad.',
    shortDescription: 'Banners, standees, flags & vinyl stickers',
    image: 'https://placehold.co/800x500/dc2626/ffffff',
    icon: 'Flag',
    seoTitle: 'Event Branding & Signage in Hyderabad | PrintNest',
    seoDescription:
      'Professional event signage in Hyderabad. Roll-up banners, standees, flags, and vinyl stickers for exhibitions and promotions.',
  },
];

export function getCategoryBySlug(slug: string): Category | undefined {
  return categories.find((c) => c.slug === slug);
}

export function getCategoryByKey(key: string): Category | undefined {
  return categories.find((c) => c.key === key);
}
